...
func RegisterRoutes(router *gin.Engine, cfg *config.Config, db *gorm.DB, oidcProvider *services.OIDCProvider) {
	...
	v1 := router.Group("/api/v1")
	{
		...
		protected.Use(middleware.AuthMiddleware(cfg))
		{
			...
			// Company routes (client companies)
			protected.GET("/contacts/:id/companies", controllers.GetCompanies)
			protected.POST("/contacts/:id/companies", middleware.ValidateJSONMiddleware(&models.CompanyInput{}), controllers.CreateCompany)
			...
			// Bulk Excel import route
			protected.POST("/contacts/import/excel", controllers.ImportExcelContacts)
