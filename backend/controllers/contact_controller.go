...
func GetContact(c *gin.Context) {
	...
	if len(selectedFields) > 0 {
		...
		// are preloaded separately.
		var preloads []string
		var columnFields []string
		for _, field := range selectedFields {
			if field == "companies" {
				preloads = append(preloads, "Companies.Types")
				continue
			}
			columnFields = append(columnFields, field)
		}
		if len(columnFields) > 0 {
			query = query.Select(columnFields)
		}
...
func UpdateContact(c *gin.Context) {
	...
	replaceCompanies := contactInput.Companies != nil
	...
	if err := db.Transaction(func(tx *gorm.DB) error {
		...
		if replaceCompanies {
			if err := tx.Where("contact_id = ?", contact.ID).Delete(&models.Company{}).Error; err != nil {
				return err
			}
			if len(contactInput.Companies) > 0 {
				for i := range contactInput.Companies {
					contactInput.Companies[i].ID = 0
					contactInput.Companies[i].ContactID = contact.ID
				}
				if err := tx.Create(&contactInput.Companies).Error; err != nil {
					return err
				}
			}
		}
	...
	// Reload with companies so the response includes them
	var updatedContact models.Contact
	if err := db.Preload("Companies").First(&updatedContact, contact.ID).Error; err == nil {
...
func DeleteContact(c *gin.Context) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		// Manually delete associated companies
		if err := tx.Where("contact_id = ?", id).Delete(&models.Company{}).Error; err != nil {
			return err
		}
