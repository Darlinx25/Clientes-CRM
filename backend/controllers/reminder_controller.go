...
func GetUpcomingReminders(c *gin.Context) {
	...
	if err := db.Where("remind_at <= ? AND completed = ?", sevenDaysFromNow, false).
		Order("remind_at ASC").
	...
	if err := db.Where("completed = ?", false).
		Order("remind_at ASC").
		Limit(5).
...
func GetCompletionsForContact(c *gin.Context) {
	...
	if err := db.Where("contact_id = ?", contactID).
...
func DeleteCompletion(c *gin.Context) {
	...
	if err := db.First(&completion, id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
