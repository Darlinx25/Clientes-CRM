...
func TestCreateUnassignedNoteWithoutContactID(t *testing.T) {
	...
	noteDate := time.Now()
	newNote := models.NoteInput{
		Content: "This is a floating note without a contact or company.",
		Date:    noteDate,
	}
...
func TestCreateNoteWithCompany(t *testing.T) {
	db, router := setupRouter()
	...
	noteDate := time.Now()
	newNote := models.NoteInput{
		Content:   "Nota ligada a una empresa.",
		Date:      noteDate,
	...
	assert.NotNil(t, response.Note.CompanyID)
	assert.Equal(t, company.ID, *response.Note.CompanyID)
}
