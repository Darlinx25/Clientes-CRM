...
- Build and run locally from this repository: `docker compose up -d --build` (the image is always built from the root [Dockerfile](Dockerfile); no upstream images are pulled). For a plain build: `docker build -t clientes-crm .`.
